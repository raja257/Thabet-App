...
const TeacherbyParentId = () => {
    const [userData, setUserData] = useState(null);
    const [teachers, setTeachers] = useState([]);
    const [chats, setChats] = useState([]);
    const router = useRouter();
    const text = "Thank you for the update. I’ll make sure Sarah reviews her notes.";
    ...
    useEffect(() => {
        const chatData = teachers.map((teacher) => ({
            id: teacher._id,
    ...
    const getParentByTeacherId = async () => {
        try {
            const response = await fetch("http://localhost:8000/user/get_teacher_by_parent_id", {
                method: "POST",
                headers: {
            ...
            const teachers = result.teacher_data.map((item) => item.teacher);
...
export default TeacherbyParentId;
